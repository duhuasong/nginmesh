...
subprocess.check_output("istioctl create -f ../nginmesh-0.2.12/samples/kubernetes/route-rule-reviews-v3.yaml > /dev/null 2>&1",universal_newlines=True,shell=True)
...
if  v1_count==0 and v2_count==0 and v3_count>0:
    print("Route all to V3  Test -- Passed")
...
subprocess.check_output("istioctl delete -f ../nginmesh-0.2.12/samples/kubernetes/route-rule-reviews-v3.yaml > /dev/null 2>&1",universal_newlines=True,shell=True)
