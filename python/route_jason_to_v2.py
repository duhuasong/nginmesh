...
subprocess.check_output("istioctl create -f ../nginmesh-0.2.12/samples/kubernetes/route-rule-reviews-test-v2.yaml > /dev/null 2>&1",universal_newlines=True,shell=True)
...
while total_count < 10:
    output = str(subprocess.check_output("curl --cookie 'user=jason' -s -w 'Status_Code=%{http_code}\n' http://"+GATEWAY_URL+"/productpage", universal_newlines=True,shell=True)).rstrip()
...
if  v1_count==0 and v2_count>0 and v3_count==0:
    print("Route 'jason' user to V2 Test -- Passed")
...
subprocess.check_output("istioctl delete -f ../nginmesh-0.2.12/samples/kubernetes/route-rule-reviews-test-v2.yaml > /dev/null 2>&1",universal_newlines=True,shell=True)
