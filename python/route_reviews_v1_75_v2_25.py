...
subprocess.check_output("istioctl create -f ../nginmesh-0.2.12/samples/kubernetes/addons/route-rule-destination-weight.yaml > /dev/null 2>&1",universal_newlines=True,shell=True)
...
if  v1_count>0 and v2_count>0 and v3_count==0 and v1_count>v2_count:
    print("Route all to V1:75% and V2:25% Test -- Passed")
...
subprocess.check_output("istioctl delete -f ../nginmesh-0.2.12/samples/kubernetes/addons/route-rule-destination-weight.yaml > /dev/null 2>&1",universal_newlines=True,shell=True)
