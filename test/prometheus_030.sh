...
kubectl apply -f ../istio-0.3.0/install/kubernetes/addons/prometheus.yaml > /dev/null 2>&1
sleep 10;
...
. performance.sh
 ...
 out=$(curl -s -X GET "http://localhost:9090/api/v1/query?query=istio_request_count" -H "accept: application/json" )
...
kubectl delete -f ../istio-0.3.0/install/kubernetes/addons/prometheus.yaml > /dev/null 2>&1
ps -ef | grep prometheus | grep -v grep | awk '{print $2}' | xargs kill -9  > /dev/null 2>&1
