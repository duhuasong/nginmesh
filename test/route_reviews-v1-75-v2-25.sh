#!/bin/sh
. delete_routes.sh
istioctl create -f ../nginmesh-0.2.12/samples/kubernetes/addons/route-rule-destination-weight.yaml > /dev/null 2>&1
sleep 5;
i=0 
j=0
k=0
...
. performance.sh
istioctl delete -f ../nginmesh-0.2.12/samples/kubernetes/addons/route-rule-destination-weight.yaml > /dev/null 2>&1
