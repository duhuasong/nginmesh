...
kubectl apply -f ../istio-0.2.12/install/kubernetes/addons/servicegraph.yaml > /dev/null 2>&1
sleep 15;
...
kubectl delete -f ../istio-0.2.12/install/kubernetes/addons/servicegraph.yaml > /dev/null 2>&1
ps -ef | grep servicegraph | grep -v grep | awk '{print $2}' | xargs kill -9  > /dev/null 2>&1
